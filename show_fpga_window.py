...
Ehphw   = 35.       # E-plane half-power half width in deg
Hhphw   = 70.       # H-plane half-power half width in deg
dellim  = 150.
delsep  = 0.5
body    = 'sun'
...
for fvis in files:
    ...
    for b, pa in enumerate(pairs):
        ...
        if (do_model):
            ...
            fig.text(0.76, 0.80, 'med(Tsys_freq.min) = %.0f K'%np.ma.median(Tsys_freq2))
            fig.text(0.76, 0.77, 'med(SEFD_freq.min) = %.2f MJy'%(np.ma.median(SEFD2D_freq2)/1e6))
