...
while(inp):
    ...
    if (k == '--s_ew'):
        ...
        nAnt = int(k)
        nRow = int(inp.pop(0))
        beam01 = float(inp.pop(0))
        beam02 = float(inp.pop(0))
...
t_start = (t_trans_utc - timedelta(hours=4)).replace(second=0, microsecond=0)
t_end = (t_trans_utc + timedelta(hours=4)).replace(second=0, microsecond=0)

ut2 = np.linspace(t_start.timestamp(), t_end.timestamp(), nSky, endpoint=False)
...
t_end += timedelta(hours=8)
x_min = t_trans_utc + timedelta(hours=tlim[0]+8)
x_max = t_trans_utc + timedelta(hours=tlim[1]+8)
...
with open(f_out_txt, 'w') as f:
    f.write('#%s %s\ntransit at %s (local time)\n#\n' % (targetname, sitename, t_trans))
...
b_start = (ut2+timedelta(hours=8))[j]
...
for i in range(nBeam1-1, 0, -1):
    ...
    b_end = (ut2+timedelta(hours=8))[j]
    if (b_end <= t_start) or (b_start >= t_end): 
        b_start = b_end
        continue
    elif (b_start < t_start): b_start = t_start
    elif (b_end > t_end): b_end = t_end
    b_start_str = (b_start+timedelta(seconds=30)).strftime('%Y%m%d_%H%M%S')#.strftime('%Y%m%d %H:%M')
    b_end_str = (b_end+timedelta(seconds=30)).strftime('%Y%m%d_%H%M%S')#.strftime('%Y%m%d %H:%M')
...
for i in [0]:
    ...
    b_start_str = (b_start+timedelta(seconds=30)).strftime('%Y%m%d_%H%M%S')
    b_end_str = (b_end+timedelta(seconds=30)).strftime('%Y%m%d_%H%M%S')#.strftime('%Y%m%d %H:%M')
    print('Beam %03d (row %d, beam %02d): %s - %s' % (max_bid*nBeam1+i, max_bid, i, b_start_str, b_end_str))
    with open(f_out_txt, 'a') as f:
