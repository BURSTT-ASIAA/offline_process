...
f218 = 'fpga218_221209_f100.bin'  # ch 8
f219 = 'fpga218_221209_f100.bin'  # ch 10
...
fh218 = open(f218, 'rb')
fh219 = open(f219, 'rb')
fullBM218 = loadFullbitmap(fh218, nBlock)
fullBM219 = loadFullbitmap(fh219, nBlock)


t0 = time.time()
print('reading fpga218:', 2000, 'packets')
pack00 = 2500000
bitmap = fullBM218[pack00:pack00+npack*2]
tick, fullspec0 = loadSpec(fh218, pack00, int(2*npack), 8192, 2, bitmap=bitmap)
spec0 = fullspec0[:,5]
norm0 = np.abs(spec0)
t1 = time.time()
...
for jj in range(nrepeat):
    ...
    for ii in range(nloop+2):
        ...
        tmp = loadSpec(fh219, pack0, npack, 8192, 2, bitmap=bitmap)
        if (tmp is not None):
            tick, spec = tmp
            fullspec1[ii] = spec[:,7]
