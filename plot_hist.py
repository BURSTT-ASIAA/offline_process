...
options are:
    ...
    --xlim XMIN XMAX    # set the plot range in x (range of histogram)
...
while inp:
    ...
    if (k == '-n'):
        ...
        xmin = float(inp.pop(0))
        xmax = float(inp.pop(0))
        xlim = [xmin, xmax]
    elif (k == '-v'):
        verbose = 1
    elif (k.startswith('-')):
...
for fii,fin in enumerate(files):
    ...
    if (xlim is None):
        xmin = min(spec.real.min(), spec.imag.min())
        xmax = max(spec.real.max(), spec.imag.max())
    ...
    for ai in range(16):
        ...
        ax.hist(spec[:,ai].real.flatten(), bins=128, range=[xmin, xmax])
        ax.hist(spec[:,ai].imag.flatten(), bins=128, range=[xmin, xmax])
