...
nChan   = 960           # not full bandwisth used
nChan1  = nChan * nOrder
chlim   = [0, nChan1]   # the default
flim    = [300., 700.]
combine = True
rows    = None
...
options are:
    ...
    --sep sep       # sapratation of windows in blocks (%d)
...
while (inp):
    ...
    if (k=='-o'):
        ...
        nChan = int(inp.pop(0))
        nChan1 = nChan * nOrder # only counting the packetized/upchannelized channels
        chlim   = [0, nChan1]   # channel limit for spectral average, override
    elif (k=='-v'):
        verbose=1
    elif (k=='--fwin'):
        nFWin = int(inp.pop(0))
    elif (k == '--sep'):
        sepWin = int(inp.pop(0))
    elif (k=='--raw'):
        pts = [0]
    elif (k=='--both'):
        pts = [0, 1]
...
# use the original channelization (up-channelized if applicable) to define frequency bins
...
for j in range(nDir):
    ...
    if (read_raw):
        ...
        fepoch  = filesEpoch(files, hdver=2, meta=0, ppf=8) # ppf=8 for bf512 mode
        epoch0  = fepoch[0] # UTC
...
hratio[0::2] = 2
hratio[1::2] = 1
fig, tmp = plt.subplots(nRow2*2,nAnt,figsize=(nAnt*2,nRow2*3), sharex=True, height_ratios=hratio)
sub  = tmp[0::2]
sub2 = tmp[1::2]
...
if (combine):
    ...
    chan1 = np.arange(arrNInt.shape[1])
    if (chlim[1] > chan1[-1]+1):
        chlim[1] = chan1[-1]+1
...
for j in range(nRow):
    fig, s2d = plt.subplots(12,6,figsize=(16,12), sharex=True, height_ratios=[2,1,2,1,2,1,2,1,2,1,2,1])
    sub = s2d[0::2,:].flatten()
    sub2 = s2d[1::2,:].flatten()
    for ii in range(6):
        s2d[ii*2,0].set_ylabel('freq (MHz)')
        s2d[ii*2+1,0].set_ylabel('amp')
        s2d[-1,ii].set_xlabel('time (sec)')
