...
f218 = 'fpga218_221208_f1000.bin'
f219 = 'fpga219_221208_f1000.bin'
...
fullBM218 = loadFullbitmap(fh218, 4)
fullBM219 = loadFullbitmap(fh219, 4)

t0 = time.time()
print('reading fpga218:', 4000, 'packets')
pack00 = 2398000
bitmap = fullBM218[pack00:pack00+4000]
tick, fullspec0 = loadSpec(fh218, pack00, int(4*npack), 8192, 2, bitmap=bitmap)
spec0 = fullspec0[:,7]
stream0 = spec2Stream(spec0, shift=False)
...
for jj in range(nrepeat):
    ...
    stream1 = np.zeros((nloop,nChan*npack)) # real stream, twice as long as the complex spec
    ...
    for ii in range(nloop):
        ...
        bitmap = fullBM219[pack0:pack0+npack]
        tick, spec = loadSpec(fh219, pack0, npack, 8192, 2, bitmap=bitmap)
        spec1[ii] = spec[:,7]
        stream1[ii] = spec2Stream(spec[:,7], shift=False)
        del spec
        gc.collect()
...
f1.savefig('tcorr2_loop%d_start%d.png' % (nloop*nrepeat, lstart))
plt.close(f1)
